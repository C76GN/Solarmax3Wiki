            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-lg shadow-lg p-6">
                ...
                <div class="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <div class="text-sm text-gray-600 mb-4">
                            ...
                            <span v-if="page.current_version_id"> · 当前版本: {{ page.currentVersion.version_number
                                }}</span>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <Link v-for="tag in page.tags" :key="tag.id" :href="route('wiki.index', { tag: tag.slug })"
                ...
                <div class="mt-10 pt-6 border-t border-gray-200">
                    ...
                    <div v-if="page.comments && page.comments.length" class="space-y-6">
                        <div v-for="comment in page.comments" :key="comment.id" class="bg-gray-50 p-4 rounded-lg">
                            <div class="flex justify-between">
                                <div class="flex items-center mb-2">
                                    <div class="font-medium">{{ comment.user.name }}</div>
                                    <span class="mx-2 text-gray-500">•</span>
                                    <div class="text-sm text-gray-500">{{ formatDateTime(comment.created_at) }}</div>
                                </div>
                            ...
                            <div v-if="comment.replies && comment.replies.length"
                                ...
                                <div v-for="reply in comment.replies" :key="reply.id" class="bg-gray-100 p-3 rounded">
                                    <div class="flex justify-between">
                                        <div class="flex items-center mb-2">
                                            <div class="font-medium">{{ reply.user.name }}</div>
                                            <span class="mx-2 text-gray-500">•</span>
                                            <div class="text-sm text-gray-500">{{ formatDateTime(reply.created_at) }}
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex space-x-2">
                                            <button
                                                ...
                                                @click="editComment(reply)" class="text-blue-600 hover:text-blue-800">
                                                <font-awesome-icon :icon="['fas', 'edit']" />
                                            </button>

                                            <button
                                                v-if="$page.props.auth.user && (reply.user_id === $page.props.auth.user.id || $page.props.auth.user.permissions.includes('wiki.moderate_comments'))"
                                                @click="deleteComment(reply)" class="text-red-600 hover:text-red-800">
                                                <font-awesome-icon :icon="['fas', 'trash']" />
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div v-if="$page.props.auth.user && $page.props.auth.user.permissions.includes('wiki.comment')"
                                class="mt-3">
                                <button v-if="replyingToCommentId !== comment.id" @click="replyToComment(comment)"
